...
export function EmailListItem({ email }: EmailListItemProps) {
  ...
  const handleImportantClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    // Since priority is used instead of isImportant, we'll use priority to determine importance state
        ...
        <div className="flex-1 min-w-0">          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center space-x-2">
              ...
              >
                ...
                className={`p-1 rounded hover:bg-gray-200 ${
                  email.priority === 'high' ? 'text-red-500' : 'text-gray-400'
                }`}
                title={email.priority === 'high' ? 'Remove importance' : 'Mark as important'}
              >
